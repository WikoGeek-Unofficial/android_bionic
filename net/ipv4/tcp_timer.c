...
static int tcp_out_of_resources(struct sock *sk, int do_reset)
{
	...
	if ((s32)(tcp_time_stamp - tp->lsndtime) > 2*(inet_csk(sk)->icsk_MaxRto ? inet_csk(sk)->icsk_MaxRto : sysctl_tcp_rto_max) || !do_reset)
		shift++;
				  ...
				  bool syn_set)
{
	...
	if (likely(timeout == 0)) {
		linear_backoff_thresh = ilog2((inet_csk(sk)->icsk_MaxRto ? inet_csk(sk)->icsk_MaxRto : sysctl_tcp_rto_max)/rto_base);

		if (boundary <= linear_backoff_thresh)
			timeout = ((2 << boundary) - 1) * rto_base;
		else
			timeout = ((2 << linear_backoff_thresh) - 1) * rto_base +
				(boundary - linear_backoff_thresh) * (inet_csk(sk)->icsk_MaxRto ? inet_csk(sk)->icsk_MaxRto : sysctl_tcp_rto_max);
	}
...
static int tcp_write_timeout(struct sock *sk)
{
	...
	if ((1 << sk->sk_state) & (TCPF_SYN_SENT | TCPF_SYN_RECV)) {
		...
		if (sock_flag(sk, SOCK_DEAD)) {
			const int alive = (icsk->icsk_rto < (icsk->icsk_MaxRto ? icsk->icsk_MaxRto : sysctl_tcp_rto_max));
...
static void tcp_probe_timer(struct sock *sk)
{
	...
	if (sock_flag(sk, SOCK_DEAD)) {
		const int alive = ((icsk->icsk_rto << icsk->icsk_backoff) < (icsk->icsk_MaxRto ? icsk->icsk_MaxRto : sysctl_tcp_rto_max));
...
#endif
		if (tcp_time_stamp - tp->rcv_tstamp > (icsk->icsk_MaxRto ? icsk->icsk_MaxRto : sysctl_tcp_rto_max)) {
			tcp_write_err(sk);
			goto out;
		}
		tcp_enter_loss(sk, 0);
...
static void tcp_synack_timer(struct sock *sk)
{
	inet_csk_reqsk_queue_prune(sk, TCP_SYNQ_INTERVAL,
				   TCP_TIMEOUT_INIT, (inet_csk(sk)->icsk_MaxRto ? inet_csk(sk)->icsk_MaxRto : sysctl_tcp_rto_max));
}
