...
//#define SUPPORT_KERNEL_BACKTRACE
...
{
	if (mod)
		MET_PRINTK("%s,%lx,%s.ko,%lx\n", taskname, pc, fname, off);
	else
		MET_PRINTK("%s,%lx,%s,%lx\n", taskname, pc, fname, off);
}

#ifdef SUPPORT_KERNEL_BACKTRACE
#if defined(__arm__)
static int report_trace(struct stackframe *frame, void *d)
{
	struct module *mod;
	off_t off;
	unsigned int *depth = d, pc = frame->pc;
...
static void kernel_backtrace(struct pt_regs * const regs)
{
#if defined(__arm__)
	int depth = 1;
...
#endif //SUPPORT_KERNEL_BACKTRACE
