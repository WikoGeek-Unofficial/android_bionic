...
static int __attribute__ ((unused)) lm3639_smbus_read_byte_block(struct i2c_client *client,
...
{
   int err = 0;
   err=down_interruptible(&flash_lock);   
   if(err<0){
      CPD_ERR("down_interruptible(%d)\n", err);
      return;
   }
...
void lm3639_flash_strobe_en(void)
{
   ...
   up(&flash_lock);
   CPD_ERR("Error(%d) lm3639_smbus_write_byte()\n", err);
}
