...
#ifdef 	GPIO_MSE_EINT_PIN
	mt_set_gpio_mode(GPIO_MSE_EINT_PIN, GPIO_MODE_00);
...
int FST_AK09911(void)
{
	...
	// TEST
	TEST_DATA(TLIMIT_NO_RST_WIA1,   TLIMIT_TN_RST_WIA1,   (int)i2cData[0],  TLIMIT_LO_RST_WIA1,   TLIMIT_HI_RST_WIA1,   &pf_total);
	TEST_DATA(TLIMIT_NO_RST_WIA2,   TLIMIT_TN_RST_WIA2,   (int)i2cData[1],  TLIMIT_LO_RST_WIA2,   TLIMIT_HI_RST_WIA2,   &pf_total);
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				value = *(int *)buff_in;
		printk(KERN_ERR "akm09911_orientation_operate SENSOR_ENABLE=%d!\n",value);
