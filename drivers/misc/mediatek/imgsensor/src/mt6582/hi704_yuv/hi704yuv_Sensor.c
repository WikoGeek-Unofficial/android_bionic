...
extern void SubCameraDigtalPDNCtrl(u32 onoff);	//LINE <> <20140906> <resolve the Bug that can't change to subCamera on little phones> Paul
...
static int CAPTUREB_FLAG = 0;
static int NIGHT_FLAG = 0;
...
//#define HI704_LOAD_FROM_T_FLASH
#ifdef HI704_LOAD_FROM_T_FLASH

static kal_uint8 fromsd = 0;
kal_uint16 HI704_write_cmos_sensor(kal_uint8 addr, kal_uint8 para);

#define HI704_OP_CODE_INI		0x00		/* Initial value. */
#define HI704_OP_CODE_REG		0x01		/* Register */
#define HI704_OP_CODE_DLY		0x02		/* Delay */
#define HI704_OP_CODE_END		0x03		/* End of initial setting. */
	...
	} HI704_initial_set_struct;

	HI704_initial_set_struct HI704_Init_Reg[1000];
	...
	u8 Hi704_Initialize_from_T_Flash()
	{
		...
		fp = filp_open("/storage/sdcard0/hi704_sd.dat", O_RDONLY , 0); 
		if (IS_ERR(fp)) { 
			printk("create file error\n"); 
			return 2;//-1; 
		} 
		else
		printk("Hi704_Initialize_from_T_Flash Open File Success\n");
		...
		while (curr_ptr < (data_buff + file_size))
		{
			...
			if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
			{
				...
				HI704_Init_Reg[i].op_code = HI704_OP_CODE_REG;

				HI704_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
				curr_ptr += 5;	/* Skip "00, 0x" */

				HI704_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
				curr_ptr += 4;	/* Skip "00);" */
			}
			else									/* DLY */
			{
				...
				curr_ptr += 4;	
				HI704_Init_Reg[i].op_code = HI704_OP_CODE_DLY;

				HI704_Init_Reg[i].init_reg = 0xFF;
				HI704_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
			}
			i++;
		...
		HI704_Init_Reg[i].op_code = HI704_OP_CODE_END;
		HI704_Init_Reg[i].init_reg = 0xFF;
		HI704_Init_Reg[i].init_val = 0xFF;
		i++;
		//for (j=0; j<i; j++)
		...
		for (j=0; j<i; j++)
		{
			if (HI704_Init_Reg[j].op_code == HI704_OP_CODE_END)	/* End of the setting. */
			{
				break ;
			}
			else if (HI704_Init_Reg[j].op_code == HI704_OP_CODE_DLY)
			{
				msleep(HI704_Init_Reg[j].init_val);		/* Delay */
			}
			else if (HI704_Init_Reg[j].op_code == HI704_OP_CODE_REG)
			{
				HI704_write_cmos_sensor((kal_uint8)HI704_Init_Reg[j].init_reg, (kal_uint8)HI704_Init_Reg[j].init_val);
			}
			else
			{
				printk("REG ERROR!\n");
...
{0x83,0x01}, //EXP Normal 10.00 fps 
{0x84,0x24}, 
{0x85,0xf8}, 
{0x86,0x00}, //EXPMin 3000.00 fps
{0x87,0xfa}, 
{0x88,0x01}, //EXP Max 5.00 fps 
{0x89,0xd4}, 
{0x8a,0xc0}, 
{0x8B,0x1d}, //EXP100 
{0x8C,0x4c}, 
...
{0x9c,0x03}, //EXP Limit 428.57 fps 
{0x9d,0x6b}, 
{0x9e,0x00}, //EXP Unit 
{0x9f,0xfa}, 
...
UINT32 HI704Close(void)
{
	 ...
	 //BEGIN <> <20130822> <resolve the Bg that can't change to subCamera on little phones> panzaoyan
	  /*
	mt_set_gpio_mode(GPIO_CAMERA_CMPDN_PIN_M_GPIO,GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_CAMERA_CMPDN_PIN_M_GPIO,GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CAMERA_CMPDN_PIN_M_GPIO,GPIO_OUT_ZERO);
...
//	mt_set_gpio_out(GPIO_CAMERA_CMPDN_PIN,GPIO_OUT_ZERO);
	...
	//END <> <20130822> <resolve the Bg that can't change to subCamera on little phones> panzaoyan
...
//BEGIN <> <20130917> <improve the camera experience that before to capture the next photo, the screen lighten instantly > panzaoyan
    ...
    HI704_write_cmos_sensor(0x8b, 0x1d);
    HI704_write_cmos_sensor(0x8c, 0x4c);
    HI704_write_cmos_sensor(0x8d, 0x18);
    HI704_write_cmos_sensor(0x8e, 0x6a);

    HI704_write_cmos_sensor(0x9c, 0x03);
    HI704_write_cmos_sensor(0x9d, 0x6b);
...
//END <> <20130917> <improve the camera experience that before to capture the next photo, the screen lighten instantly > panzaoyan
}
...
void HI704_night_mode(kal_bool enable)
{
    ...
    if(HI704_sensor.MPEG4_Video_mode == KAL_TRUE)
        return;
	NIGHT_FLAG = 0; //LINE <> <20130917> <improve the camera experience that before to capture the next photo, the screen lighten instantly > panzaoyan
...

{
  ...
  HI704_write_cmos_sensor(0x85, 0x90);//15fps
  HI704_write_cmos_sensor(0x86, 0x00);
  HI704_write_cmos_sensor(0x87, 0xfa);
  ...
  HI704_write_cmos_sensor(0x8A, 0x6c);//7
 ...
 //BEGIN <> <20130922> <improve the camera experience that before to capture the next photo, the screen lighten instantly> panzaoyan
        ...
        HI704_write_cmos_sensor(0x83, 0x01);
        HI704_write_cmos_sensor(0x84, 0x6e);
        HI704_write_cmos_sensor(0x85, 0x36);//15fps
		HI704_write_cmos_sensor(0x86, 0x00);
		HI704_write_cmos_sensor(0x87, 0xfa);
 //END <> <20130922> <improve the camera experience that before to capture the next photo, the screen lighten instantly> panzaoyan		
		HI704_write_cmos_sensor(0x88, 0x02);
...
else
{
  ...
  HI704_write_cmos_sensor(0x8A, 0xc0);//7
  ...
  HI704_write_cmos_sensor(0x87, 0xfa);
    //END<> <20130922> <improve the camera experience that before to capture the next photo, the screen lighten instantly> panzaoyan
  HI704_write_cmos_sensor(0x88, 0x01);
  HI704_write_cmos_sensor(0x89, 0xe8);
  HI704_write_cmos_sensor(0x8A, 0x48);//7
...
BOOL HI704_set_param_wb(UINT16 para)
{
    ...
    case AWB_MODE_AUTO:
        ...
        HI704_write_cmos_sensor(0x01, 0xd0);//OK
    ...
    case AWB_MODE_CLOUDY_DAYLIGHT:
    {
        ...
        HI704_write_cmos_sensor(0x10, 0xfb);
        //HI704_write_cmos_sensor(0x01, 0x50);//NG
    ...
    case AWB_MODE_DAYLIGHT:
       ...
       HI704_write_cmos_sensor(0x03, 0x22);
        ...
        HI704_write_cmos_sensor(0x10, 0xfb);
        //HI704_write_cmos_sensor(0x01, 0xc0);//OK FANYE OK
        }      
        break;
    case AWB_MODE_INCANDESCENT:	
       ...
       HI704_write_cmos_sensor(0x03, 0x22);
        ...
        HI704_write_cmos_sensor(0x10, 0xfb);
        //HI704_write_cmos_sensor(0x01, 0x70);//NG
        }		
        break;  
...
BOOL HI704_set_param_effect(UINT16 para)
{
   SENSORDB("[Enter]HI704 set_param_effect func:para = %d\n",para);
    ...
    case MEFFECT_SEPIA:
		...
		//HI704_write_cmos_sensor(0x03, 0x00);
        //HI704_write_cmos_sensor(0x12, 0x01);
        }	
        break;  
...
BOOL HI704_set_param_banding(UINT16 para)
{
    ...
    case AE_FLICKER_MODE_50HZ:
		...
		//HI704_write_cmos_sensor(0x88, 0x01);
		//HI704_write_cmos_sensor(0x89, 0xd4);
		//HI704_write_cmos_sensor(0x8A, 0xc0);//50HZ
    ...
    //HI704_write_cmos_sensor(0x88, 0x01);
   // HI704_write_cmos_sensor(0x89, 0x9f);
    //HI704_write_cmos_sensor(0x8A, 0x0a);//60HZ
...
BOOL HI704_set_param_exposure(UINT16 para)
{
    ...
    case AE_EV_COMP_00:  // +0 EV
        HI704_write_cmos_sensor(0x40,0x82);
        break;    
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
  ...
  #ifndef HI704_LOAD_FROM_T_FLASH
