...
#define OTP_MID               0x0d//    0x02
#define OTP_KMJ_MID		0x08
...
static signed char otp_check_wb_group(unsigned char index)
{
    ...
    else if (((!(flag&0x80)) && ((flag&0x7f) == OTP_MID)) || ((!(flag&0x80)) && ((flag&0x7f) == OTP_KMJ_MID)))
    {
...
signed char check_cmk_MID(signed char index)
{
    ...
    for (index=0; index<3; index++)
    {
        if (otp_check_wb_group(index) == 2)
        {
            ...
            TRACE("check_MID sucessed %d", index);
