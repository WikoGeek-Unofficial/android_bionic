...
extern kal_uint16 OV5648MIPI_write_cmos_sensor_darling(kal_uint32 addr, kal_uint32 para);
...
#define OV5648_write_cmos_sensor(addr,para) OV5648MIPI_write_cmos_sensor_darling(addr,para)
#define OV5648_read_cmos_sensor(addr) OV5648MIPI_read_cmos_sensor_darling(addr)
...
#define OTP_MID               0x05
#define ZL_LENS_ID              0x02 //ÖÐÀ¶Âí´ï
...
static signed char otp_check_wb_group(unsigned char index)
{
    ...
    otp_read(otp_addr+1, &lens_id);
    otp_clear();

    TRACE("otp_check_wb_group flag: 0x%x", flag);
    TRACE("otp_check_wb_group lens_id: 0x%x", lens_id);
    ...
    else if ((!(flag&0x80)) && ((flag&0x7f) == OTP_MID) && lens_id !=ZL_LENS_ID)
    {
...
/*******************************************************************************
* Function    :  otp_update_wb_darling
* Description :  Update white balance settings from OTP
* Parameters  :  [in] golden_rg : R/G of golden camera module
                 [in] golden_bg : B/G of golden camera module
* Return      :  1, success; 0, fail
*******************************************************************************/
bool otp_update_wb_darling(unsigned short golden_rg, unsigned short golden_bg)
{
...
signed char check_darling_MID(signed char index)
{
    ...
    for (index=0; index<3; index++)
    {
        if (otp_check_wb_group(index) == 2)
        {
            TRACE("check_darling_MID sucessed %d", index);
    ...
    TRACE("check_darling_MID failed");
