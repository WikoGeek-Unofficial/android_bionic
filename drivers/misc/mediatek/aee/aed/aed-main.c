...
static void ee_gen_detail_msg(void)
{
	...
	if (strncmp(eerec->assert_type, "md32", 4) == 0) {
		rep_msg = msg_create(&eerec->msg, (eerec->ee_log_size + 64));
		if (rep_msg == NULL)
			return;
		...
		rep_msg = msg_create(&eerec->msg, (eerec->ee_log_size * 4));
		if (rep_msg == NULL)
			return;
		...
		if (mem) {
			...
			n += sprintf(data + n, "kmalloc fail, no log available\n");
		}
	}
...
{
	int *ee_log = NULL;
	struct aed_eerec *eerec = kzalloc(sizeof(struct aed_eerec), GFP_ATOMIC);
