...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    if(isAEEEnabled)
    {
        MTKFB_WRAN("[FB Driver] aee on so changed pan_display to layer %d \n",DISP_CHANGED_UI_LAYER_ID);
...
static int mtkfb_set_overlay_layer(struct fb_info *info, struct fb_overlay_layer* layerInfo, unsigned int locked)
{
    ...
    if (!enable)
    {
        ...
        {
            MTKFB_INFO("mtkfb update fence from %d to %d",temp[id].buff_idx,max_buf_index_setted[id]);
        ...
        max_buf_index_setted[id] = temp[id].buff_idx;
    {
        ...
        MTKFB_INFO("Totally %d security layer is set now\n", cnt_security_layer);
...
void mtkfb_dump_layer_info(void)
{
  ...
  MTKFB_INFO("[mtkfb] captured(current): \n");
	for(i=0;i<4;i++)
	{
		MTKFB_INFO("[mtkfb] layer=%d, layer_en=%d, idx=%d, fmt=%d, addr=0x%x, %d, %d, %d \n ", 
