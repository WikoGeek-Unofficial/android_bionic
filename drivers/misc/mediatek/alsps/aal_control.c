...
{ 
		...
		switch (cmd)
		{
			...
			case AAL_SET_ALS_MODE:
			    ...
			    if (alsps_aal_get_status())  
				{
				old_arch = 0;
				break; 
			    }
				if (hwmsen_aal_get_status())
				{
				old_arch = 1;
				break;
				}
			
				if((err = alsps_aal_enable(enable)))
				{
					AAL_LOG("als driver don't support new arch, goto execute old arch: %ld\n", err); 
				    if((err = hwmsen_aal_enable(enable)))
						AAL_ERR("Enable als driver fail %ld\n", err);
					else 
						{
						old_arch = 1;
						AAL_LOG("Enable als driver success\n");
						}
				}
				else
					{
