...
static int kxtik1004_probe(void);
static int kxtik1004_remove(void);

static int kxtik1004_init_flag = 0;

static struct sensor_init_info kxtik1004_init_info = {
		.name = "kxtik1004",
		.init = kxtik1004_probe,
...
/*----------------------------------------------------------------------------*/
#if defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
int kxtik1004_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static long kxtik1004_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		#if 0//defined(AGOLD_ACC_DIRECTION_TOOL)
		case GSENSOR_IOCTL_GET_ACC_DIRECTION:
			{
				int acc_direction = 0;
				data = (void __user*)arg;
				if(data == NULL)
				...
				#endif
				acc_direction = obj->hw->direction;
				printk("[Agold spl][GSENSOR_IOCTL_GET_ACC_DIRECTION] acc_direction = %d\n",acc_direction);
				if(copy_to_user(data, &acc_direction, sizeof(acc_direction)))
				{
					err = -EFAULT;
					break;
				}	
			...
			break;
		case GSENSOR_IOCTL_SET_ACC_DIRECTION:
			{
				int acc_direction = 0;
				data = (void __user*)arg;
				if(data == NULL)
				...
				if(copy_from_user(&acc_direction, data, sizeof(acc_direction)))
				{
					err = -EFAULT;
					break;	  
				}
				printk("[Agold spl][GSENSOR_IOCTL_SET_ACC_DIRECTION] acc_direction = %d\n",acc_direction);
				if((err = hwmsen_get_convert(acc_direction, &obj->cvt)))
				{
					printk("[Agold spl][ioctl] hwmsen_get_convert Error.\n");
					break;
				}
			}
			break;
...
static int kxtik1004_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    sobj.polling = 1;
	#if defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
	sobj.sensor_operate = kxtik1004_operate;
	#else
...
#endif
{
	...
	if(-1 == kxtik1004_init_flag)
	{
		...
		printk("Eric trace: kxtik1004 init client error, need to remove driver!\n");
		return -1;
	}
	#endif
...
static int __init kxtik1004_init(void)
{
	...
	#if defined(CONFIG_MTK_AUTO_DETECT_ACCELEROMETER)
	if(hwmsen_gsensor_add(&kxtik1004_init_info))
	{
