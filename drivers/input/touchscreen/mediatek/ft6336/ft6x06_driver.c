...
static int tpd_enable_ps(int enable)
{
    ...
    if (enable)
    {
        ...
        printk("[proxi_5206]ps function is on\n");
    }
    else
    {
        state &= 0x00;
        tpd_proximity_flag = 0;
        printk("[proxi_5206]ps function is off\n");
    }

    ret = i2c_smbus_write_i2c_block_data(g_pts->client, TPD_PROXIMITY_ENABLE_REG, 1, &state);
    printk("[proxi_5206]write: 0xB0's value is 0x%02X\n", state);
    return 0;
}
...
static int touch_event_handler(void *para)
{
   ...
   // kal_uint8 chr_status = 0;//yixuhong 20141022 add,check for charger status
    ...
    do
    {
        ...
        //BEGIN <touch panel> <DATE20130831> <tp proximity> zhangxiaofei
#if defined TPD_PROXIMITY
        printk(" proxi_tpd_proximity_flag0x%02X\n", tpd_proximity_flag);
...
{
    TPD_DEBUG("[Focaltech] enter update_firmware_thread tpd_desc =%s\n",tpd_desc);
    //CTP_DBG("Touch Panel tpd_desc zx (%s) Probe PASS\n", tpd_desc);
...
//  mt_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_SENSITIVE);
    ...
    //mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_TOUCH_PANEL_POLARITY, tpd_eint_interrupt_handler, 0);
