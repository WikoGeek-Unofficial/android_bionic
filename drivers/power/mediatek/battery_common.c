...
static void mt_battery_CheckBatteryStatus(void)
{
    ...
    {
        BMT_status.bat_charging_state = CHR_ERROR;
		//yixuhong 20140926 add start,turn off charging led when charging error
		battery_xlog_printk(BAT_LOG_CRTI, "[Jacky debug] mt_battery_CheckBatteryTemp error! %d\n", __LINE__);
		mt65xx_leds_brightness_set(0/*MT65XX_LED_TYPE_RED*/, 0/*LED_OFF*/);
		mt65xx_leds_brightness_set(1/*MT65XX_LED_TYPE_GREEN*/, 0/*LED_OFF*/);
		//yixuhong 20140926 add end,turn off charging led when charging error
        return;                  
    }	  
...
static ssize_t current_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int len = 0;
	char desc[32];
	U32 cmd_current_unlimited = false;
