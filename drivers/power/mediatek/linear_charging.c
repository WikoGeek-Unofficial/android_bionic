...
kal_uint32 get_charging_setting_current()
{
 ...
 static void mtk_ta_BJT_check(void)
 {
	...
	curr_vbat = battery_meter_get_battery_voltage();
...
static kal_uint32 charging_full_check(void)
{
    ...
    #endif /* CONFIG_MTK_SNB1058_SUPPORT */
#endif
...
PMU_STATUS BAT_PreChargeModeAction(void)
{
...
#if defined(MTK_PUMP_EXPRESS_SUPPORT)//defined(MTK_LINEAR_CHARGER_NO_DISCHARGE)    
	// no disable charging
#else
	{
    ...
    #if defined (CONFIG_MTK_SNB1058_SUPPORT)
	if(BMT_status.total_charging_time == BAT_TASK_PERIOD)
    #endif /* CONFIG_MTK_SNB1058_SUPPORT */
