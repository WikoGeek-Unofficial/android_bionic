...
rx_submit(struct eth_dev *dev, struct usb_request *req, gfp_t gfp_flags)
{
	...
	*/
	...
	pr_debug("%s: size: %d, mtu: %d, header_len: %d, maxpacket: %d, ul_max_pkts_per_xfer: %d",
		__func__, size, dev->net->mtu, dev->port_usb->header_len, out->maxpacket, dev->ul_max_pkts_per_xfer);
...
static void rx_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case -ESHUTDOWN:		/* disconnect etc */
		...
		xlog_printk(ANDROID_LOG_INFO, UETHER_LOG, "rx shutdown, code %d\n", status);
		goto quiesce;

	/* for hardware automagic (such as pxa) */
	case -ECONNABORTED:		/* endpoint reset */
		DBG(dev, "rx %s reset\n", ep->name);
		xlog_printk(ANDROID_LOG_INFO, UETHER_LOG, "rx %s reset\n", ep->name);
		defer_kevent(dev, WORK_RX_MEMORY);
quiesce:
		dev_kfree_skb_any(skb);
		goto clean;
	...
	default:
		...
		dev->net->stats.rx_errors++;
		DBG(dev, "rx status %d\n", status);
		xlog_printk(ANDROID_LOG_INFO, UETHER_LOG, "rx_errors, status %d\n", status);
		break;
	}
					...
					struct net_device *net)
{
	...
	if (!in) {
		dev_kfree_skb_any(skb);
		xlog_printk(ANDROID_LOG_INFO, UETHER_LOG, "eth_start_xmit, but no in ep\n");
		return NETDEV_TX_OK;
	}
	...
	if (!is_promisc(cdc_filter)) {
		...
		if (is_multicast_ether_addr(dest)) {
			...
			if (!(cdc_filter & type)) {
				dev_kfree_skb_any(skb);
				xlog_printk(ANDROID_LOG_INFO, UETHER_LOG, "cdc_filter error, cdc_filter is 0x%x , type is 0x%x \n", cdc_filter , type);
				return NETDEV_TX_OK;
			}
		}
...
struct net_device *gether_connect(struct gether *link)
{
	...
	xlog_printk(ANDROID_LOG_INFO, UETHER_LOG, "gether_connect !!\n");
...
void gether_disconnect(struct gether *link)
{
	...
	DBG(dev, "%s\n", __func__);
	xlog_printk(ANDROID_LOG_INFO, UETHER_LOG, "gether_disconnect !!\n");
