...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	if (sc->nr_to_scan <= 0 || min_score_adj == OOM_SCORE_ADJ_MAX + 1) {
		...
		if (in_lowmem) {
			in_lowmem = 0;
			//DAL_LowMemoryOff();
			lowmem_print(1, KERN_DEBUG "LowMemoryOff\n");
		}
#endif
	...
	if (print_extra_info) {
		lowmem_print(1, KERN_DEBUG "======low memory killer=====\n");
		lowmem_print(1, KERN_DEBUG "Free memory other_free: %d, other_file:%d pages\n", other_free, other_file);
	}		
#endif
	...
	for_each_process(tsk) {
		...
		if (print_extra_info) {
#ifdef CONFIG_SWAP
			lowmem_print(1, KERN_DEBUG "Candidate %d (%s), oom_score_adj %d, oom_adj %d, rss %lu, rswap %lu, to kill\n",
				     p->pid, p->comm, oom_score_adj, REVERT_ADJ(oom_score_adj), get_mm_rss(p->mm),
				     get_mm_counter(p->mm, MM_SWAPENTS));
#else // CONFIG_SWAP
			lowmem_print(1, KERN_DEBUG "Candidate %d (%s), oom_score_adj %d, oom_adj %d, rss %lu, to kill\n",
				     p->pid, p->comm, oom_score_adj, REVERT_ADJ(oom_score_adj), get_mm_rss(p->mm));
#endif // CONFIG_SWAP
        }
#endif // CONFIG_MT_ENG_BUILD
...
#ifdef CONFIG_MT_ENG_BUILD
		if (print_extra_info) {
			lowmem_print(1, KERN_DEBUG "low memory info:\n");
			show_free_areas_minimum();
        #ifdef CONFIG_ION_MTK
			/* Show ION status */
			lowmem_print(1, KERN_DEBUG "ion_mm_heap_total_memory[%ld]\n", (unsigned long)ion_mm_heap_total_memory());
...
#define MSG_SIZE_TO_AEE 70
		...
		if (!in_lowmem && selected_oom_score_adj <= lowmem_debug_adj) {
			in_lowmem = 1;
			//DAL_LowMemoryOn();
			lowmem_print(1, KERN_DEBUG "LowMemoryOn\n");
